...
/*   Updated: 2022/05/10 22:30:22 by Loui :)          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_philos(t_philo *philo, t_vars *vars, int nr_philos)
{
	...
	while (i < nr_philos)
	{
		philo[i].id = i + 1;//any var you are changing needs a mutex. This does not change
		philo[i].vars = vars;
		philo[i].r_philos = &philo[(i + nr_philos - 1) % nr_philos];
		philo[i].l_philos = &philo[(i + 1) % nr_philos];
		philo[i].l_fork = ON_THE_TABLE;
		//right fork accessed thru ptr to r_philo's left fork :)
		philo[i].forks_in_hand = 0; //if fih == 2, eat
		philo[i].vars->meal_limit = vars->meal_limit;
		philo[i].start_eat_time = 0; //to calc when to die

		philo[i].d_o_a = ALIVE;
		//all of these mutexes are specific to the indiv. philo
...
int	ft_create_threads(t_philo *philo)
{
	...
	while (i < philo->vars->nr_philos)
	{
		if (pthread_join(philo[i].thread, NULL) != 0)//do I need an error msg here?
			return (1);
		i++;
	}
	return (0);
